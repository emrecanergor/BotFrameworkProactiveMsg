...
namespace ProactiveTeamsBot.Bots
{
    public class ProactiveBot : ActivityHandler
    {
        ...
        public async Task<ConversationResourceResponse> CreateConversation(string message, ITurnContext turnContext )
        {
            ...
            await turnContext.SendActivityAsync(MessageFactory.Text($"channelData: {channelData?.Channel} ___ eventType: {channelData?.EventType} ___ notification: {channelData?.Notification} ___ team: {channelData?.Team} ___ tenant: {channelData?.Tenant} "));
            await turnContext.SendActivityAsync(MessageFactory.Text($"recipientName: {turnContext?.Activity?.Recipient?.Name} "));
